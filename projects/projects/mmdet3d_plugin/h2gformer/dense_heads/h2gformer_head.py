...
from projects.mmdet3d_plugin.h2gformer.utils.header import Header
from projects.mmdet3d_plugin.h2gformer.utils.ssc_loss import sem_scal_loss, KL_sep, geo_scal_loss, CE_ssc_loss, IoE_PA_loss
from projects.mmdet3d_plugin.models.utils.bricks import run_time
import copy
import pickle

@HEADS.register_module()
class H2GFormerHead(nn.Module):
    def __init__(
        self,
        *args,
        bev_h,
    ...
    def step(self, out_dict, target, img_metas, step_type):
        ...
        if step_type== "train":
            ...
            if self.sem_scal_loss:
                ...
                loss_IoE_PA = IoE_PA_loss(ssc_pred, target)
                loss_dict['loss_IoE_PA'] = loss_IoE_PA
            ...
            # self.generate_outputs(img_metas, result)
    ...
    def save_pred(self, img_metas, y_pred):
        ...
        # save predictions
        pred_folder = os.path.join("./h2gformer", "sequences", img_metas[0]['sequence_id'], "predictions") 
        if not os.path.exists(pred_folder):
            os.makedirs(pred_folder)
        y_pred_bin = y_pred.astype(np.uint16)
        y_pred_bin.tofile(os.path.join(pred_folder, img_metas[0]['frame_id'] + ".label"))
    
    def generate_outputs(self, img_metas, result):
        output_dir = os.path.join('outputs/h2gformer-T', 'semantic_kitti')
        save_dir = os.path.join(output_dir, img_metas[0]['sequence_id'])
        file_path = os.path.join(save_dir, img_metas[0]['frame_id'] + '.pkl')
        # output_dict = copy.deepcopy(result)
        output_dict = dict()
        output_dict['pred'] = result['y_pred']
        output_dict['target'] = result['y_true']

        for key in ['T_velo_2_cam', 'fov_mask_1']:
            output_dict[key] = img_metas[0][key]
